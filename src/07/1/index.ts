...
// const filename = './src/07/1/example.txt';
const filename = './src/07/1/puzzle-input.txt';
...
const hands = lines
  .map((line) => {
    ...
    return {
      ...
      type: getHandType(match[1].split('')),
      bid: +match[2],
    };
  })
  .sort((a, b) => {
...
console.log(`What are the total winnings? ${winnings}`);
